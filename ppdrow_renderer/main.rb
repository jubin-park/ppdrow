...
class Welcome < (Example rescue Gosu::Window)
  ...
  def initialize
    super WIDTH, HEIGHT
    button_file = File.new('D:/dev/GitHub/ppdrow/samples/UI_shop.bmp_00', 'rb')
    ...
    cursor_file = File.new('D:/dev/GitHub/ppdrow/samples/기본몸.bmp_00', 'rb')
    cursor_size = cursor_file.read(4).bytes
    @cursor_width = cursor_size[0] + (cursor_size[1] << 8)
    @cursor_height = cursor_size[2] + (cursor_size[3] << 8)
    @cursor_data = cursor_file.read.bytes
